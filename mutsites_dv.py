...
def readBedFile(bed_file):
    ...
    while True:
        ...
        if not line: break
        strr = line.split()
        bed.append([strr[0], int(strr[1]), int(strr[2])])
    f.close()
    return bed
...
def getActBase(base, base_q, cigar):
    ...
    for i in range(0, len(inf), 2):
        ...
        if inf[i+1] == 'M':
            ...
            act_base.extend(str(base[poi_base:poi_base+l]).lower())
            act_q.extend(base_q[poi_q:poi_q+l])
            poi_base += l
            poi_q += l
        ...
        while i < len(base):
            if base[i] != '-' and ref[i] != '*':
                ...
                while base[i+1] == '-':
                    ...
                    i += 1
                    ref_now += ref[i]
                if chr + '_' + str(pos + 1) + '_' + ref[pos-start_point] + '_-' + ref[pos-start_point+1: i+1] in sites:
                    sites[chr + '_' + str(pos + 1) + '_' + ref[pos-start_point] + '_-' + base[pos-start_point+1: i+1]] += 1
                else:
                    sites[chr + '_' + str(pos + 1) + '_' + ref[pos-start_point] + '_-' + base[pos-start_point+1: i+1]] = 1
                pos += del_len
            else:
                ...
                while ref[i+1] == '*':
                    ...
                    i += 1
                    base_now += base_now[i]
                if chr + '_' + str(pos + 1) + '_' + ref[pos-start_point] + '_+' + base[pos-start_point+1: i+1] in sites:
                    sites[chr + '_' + str(pos + 1) + '_' + ref[pos-start_point] + '_+' + base[pos-start_point+1: i+1]] += 1
                else:
                    sites[chr + '_' + str(pos + 1) + '_' + ref[pos-start_point] + '_+' + base[pos-start_point+1: i+1]] = 1
            i += 1
...
def multiprocess(bed_arr, id):
    ...
    for i in range(core_num):
        ...
        p.close()
        p.join()
        for sites in sites_iter:
            fw.write(sites[0] + '\t' + str(sites[1]) + '\t' + sites[2] + '\t' + sites[3] + '\t' + sites[4] + '\n')
    fw.close()
