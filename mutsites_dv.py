        ...
        while i < len(base):
            if base[i] != '-' and ref[i] != '*':
                ...
                while i + 1 < len(base) and base[i+1] == '-':
                    ...
                    i += 1
                    del_part += '-'
                if chr + '_' + str(pos + 1) + '_' + ref[mut_pos] + '_-' + del_part in sites:
                    sites[chr + '_' + str(pos + 1) + '_' + ref[mut_pos] + '_-' + del_part] += 1
                else:
                    sites[chr + '_' + str(pos + 1) + '_' + ref[mut_pos] + '_-' + del_part] = 1
                pos += del_len
            else:
                ...
                while i + 1 < len(base) and ref[i+1] == '*':
                    ...
                    i += 1
                    in_part += base[i].upper()
                if chr + '_' + str(pos + 1) + '_' + ref[mut_pos] + '_+' + in_part in sites:
                    sites[chr + '_' + str(pos + 1) + '_' + ref[mut_pos] + '_+' + in_part] += 1
                else:
                    sites[chr + '_' + str(pos + 1) + '_' + ref[mut_pos] + '_+' + in_part] = 1
            i += 1

def calSitesDepth(sites):
    for s in sites.keys():
        if sites[s] <= read_count_filter:
            del sites[s]
...
def multiprocess(bed_arr, id):
    ...
    p = Pool(len(bed_arr))
    sites_list = p.map(getMutSites, bed_arr)
    p.close()
    p.join()
    for sites_arr in sites_list:
        for sites in sites_arr:
            site = sites.split('_')
            fw.write(site[0] + '\t' + str(site[1]) + '\t' + site[2] + '\t' + site[3] + '\n')
    fw.close()
