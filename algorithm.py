...
def data(file):
    ...
    while True:
        ...
        if len(stri[3]) == 1:
            arr = arr_snp
            inf = inf_snp
            if float(stri[-1]) < -3 or float(stri[-2]) > -8: continue
        poi = stri[1]
        out = chr + '\t' + poi
        inf.append(out)
        cstr = stri[4:]
        for i in range(len(cstr)):
            ...
            cstr[i] = float(cstr[i])
        arr.append(cstr)
    arr_snp = np.array(arr_snp)
    arr_indel = np.array(arr_indel)
    return arr_snp, arr_indel, inf_snp, inf_indel

def judge(id, arr, inf, cutoff, mut_type):
    clf = joblib.load(mut_type + '.m')
    y_pred = clf.predict_proba(arr)
    f = open(id + '.' + mut_type, 'w')
    for i in range(len(y_pred)):
        if y_pred[i][1] >= cutoff:
            f.write(inf[i] + '\n')
    f.close()

def validate(id, file, snp_cutoff, indel_cutoff):
    arr_snp, arr_indel, inf_snp, inf_indel = data(file)
    judge(id, arr_snp, inf_snp, snp_cutoff, 'snp')
    judge(id, arr_indel, inf_indel, indel_cutoff, 'indel')
...
def getSample(file):
    ...
    while True:
        ...
        cp = chr + '_' + cstr[1]
        chr = int(chr)
        if float(cstr[-1]) < -3 or float(cstr[-2]) > -8: continue
        cstr = cstr[4:]
        for i in range(len(cstr)):
            if cstr[i] == 'N':
                cstr[i] = 0
...
def train(mut_type, cutoff, is_train):
    ...
    fr = open(mut_type + '_result.xls', 'w')
    fw = open(mut_type + '_false.xls', 'w')
    p = 0; n = 0
    tp = 0; fp = 0
    for i in range(len(y_pred)):
...
if mut_type == 'snp':
    ...
    cutoff = indel_cutoff
train(mut_type, cutoff, is_train)
validate('NA24385', 'NA24385.mutations', snp_cutoff, indel_cutoff)