...
def ifMut(base, ref, k, start_point, cigar, type, alt):
    inf = re.split(r'(\D)', cigar)
    inf = inf[0:len(inf)-1]
    if type == 'insert' or type == 'del':
        if 'I' in cigar or 'D' in cigar:
            ...
            for i in range(0, len(inf)-2, 2):
                ...
                if inf[i+1] == 'M' or inf[i+1] == 'D':
                    summ += l
                    if summ == diff:
                        if type == 'insert' and inf[i+3] == 'I':
                            if base[diff: diff+int(inf[i+2])] == alt:
                                return 1
                        if type == 'del' and inf[i+3] == 'D' and int(inf[i+2]) == len(alt):
                            return 1
                        return 0
        return 0
    else:
...
def genReadArr(pic_start, pic_end, start_point, k, base, base_q, read_q, ref, strand, cigar, type, alt):
    read_arr = []
    for i in range(pic_start, pic_end):
        ...
        if ifMut(base, ref, k, start_point, cigar, type, alt):
...
def genReadMatrix(bamfile, chr, k, type, alt):
    arr = arrtemp = []
    arr_ref = [0] * width
    for read in bamfile.fetch(chr, k, k + 1):
        ...
        if 'I' in cigar:
            ref = recall(base, ref)
        read_arr = genReadArr(pic_start, pic_end, start_point, k, base, base_q, read_q, ref, strand, cigar, type, alt)
...
def code(bam_file, file, path):
    ...
    while True:
        ...
        if len(ref) != len(alt):
            ...
            alt = alt[1:]
        tp = int(strr[-1])
        arr = genReadMatrix(bamfile, chr, k, type, alt)
        if len(arr) < 1:
            continue
        data.append(arr)
        y.append(tp)
