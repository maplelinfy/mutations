...
def if_mut(base, ref, k, start_point, cigar, alt):
...
def genArr(bamfile, chr, k, alt):
    arr = arrtemp = arr_p = arr_n = []
    arr_ref = [0] * width
    for read in bamfile.fetch(chr, k, k + 1):
        ...
        if 'I' in cigar:
            ref = recall(base, ref)
        arrt = arr_n
        if if_mut(base, ref, k, start_point, cigar, alt):
            arrt = arr_p
        read_arr = []
        for i in range(pic_start, pic_end):
            ...
            if if_mut(base, ref, k, start_point, read.cigarstring, alt):
        ...
        arrt.append(read_arr)
    arr_ref = buildRef(arr_ref)
    arrtemp.extend(arr_p)
    arrtemp.extend(arr_n)
    depth = len(arrtemp)
